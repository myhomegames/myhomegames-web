...
export default function PublisherDetailPage({
  ...
  const [publisher, setPublisher] = useState<CollectionInfo | null>(null);
  ...
  useEffect(() => {
    if (publisherId) {
      const found = allPublishers.find((p) => String(p.id) === String(publisherId));
      if (found) {
        setPublisher({
          id: String(found.id),
          title: found.title,
  ...
  }, [publisherId, allPublishers]);

  useEffect(() => {
    if (publisherId && !fetchingRef.current) {
      fetchPublisherGames(publisherId);
    }
  }, [publisherId]);

  async function fetchPublisherInfo(id: string) {
    try {
      const url = buildApiUrl(API_BASE, `/publishers/${id}`);
  ...
  async function fetchPublisherGames(id: string) {
    fetchingRef.current = true;
    setLoading(true);
    try {
      const url = buildApiUrl(API_BASE, `/publishers/${id}/games`);
      const res = await fetch(url, {
        headers: { Accept: "application/json", "X-Auth-Token": getApiToken() || "" },
      });
  ...
  }, [isLoading, publisher, games.length]);
  ...
  if (!publisherId) {
    return (
      <div className="bg-[#1a1a1a] text-white flex items-center justify-center" style={{ width: "100%", height: "100%" }}>
        <div className="text-gray-400">{t("tags.noItemsFound", { type: t("igdbInfo.publishers", "Publishers") })}</div>
      </div>
    );
  }

  const coverUrl = publisher?.cover ? buildCoverUrl(API_BASE, publisher.cover, true) : "";
  const coverWidth = 240;
  const coverHeight = 360;
  const backgroundUrl = buildBackgroundUrl(API_BASE, publisher?.background);
  const hasBackground = Boolean(backgroundUrl && backgroundUrl.trim() !== "");

  return (
    <BackgroundManager backgroundUrl={backgroundUrl} hasBackground={hasBackground} elementId={publisherId || ""}>
              ...
              {publisher && (
                <div className="pt-8" style={{ display: "flex", flexDirection: "row", gap: "48px", alignItems: "flex-start", width: "100%", boxSizing: "border-box", marginBottom: "32px" }}>
                  ...
                  <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "flex-start", gap: "16px", minHeight: `${coverHeight}px`, minWidth: 0, visibility: "visible" }}>
                    ...
                    {(onPlay && sortedGames.some((g) => g.executables?.length)) || publisher ? (
                    ...
                    {publisher && (
                      <EditCollectionLikeModal
                        ...
                        onItemUpdate={(updated) => {
                          ...
                          updatePublisher({ ...updated, gameCount: sortedGames.length });
                        }}
                      />
                    )}
                    {publisher.summary && <Summary summary={publisher.summary} maxLines={4} />}
                    </div>
                  </div>
                </div>
              )}
