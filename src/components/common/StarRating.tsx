...
type StarRatingProps = {
  ...
  /**
   * When set, the control is a single TV/desktop focus target that opens an overlay
   * instead of editing stars in place (Smart TV detail via skin `web.tvStarRatingOverlay`).
   */
  onOpenOverlay?: () => void;
};
...
export default function StarRating({
  ...
  const interactive = !readOnly && !opensOverlay && typeof onRatingChange === "function";
  ...
  if (opensOverlay) {
    return (
      <div
        className="star-rating star-rating--overlay-trigger"
        style={rowStyle}
        role="button"
        tabIndex={0}
        data-mhg-tv-focus=""
