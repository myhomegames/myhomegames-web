  ...
  const addGame = async (catalogGame: CatalogGame): Promise<any | null> => {
    ...
    try {
      ...
      const addedGame: GameItem = {
        ...
        developers: mergeDeveloperPublisherRefsForGame(json.game?.developers, catalogDevelopers) ?? undefined,
        publishers: mergeDeveloperPublisherRefsForGame(json.game?.publishers, catalogPublishers) ?? undefined,
