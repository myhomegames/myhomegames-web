...
export default function ProfilePanelContent({ variant = "page" }: ProfilePanelContentProps) {
  ...
  if (!user && !hasCloudflareProfile) {
    return (
      <div
        className={[
          "profile-page",
          inDropdown ? "profile-page--in-dropdown" : "",
        ]
          .filter(Boolean)
          .join(" ")}
      >
  ...
  return (
    ...
    >
      <div className="profile-container">
        ...
        {hasCloudflareProfile && cloudflareProfile && (
          <div className={`bg-[#1a1a1a] profile-card${user ? " mt-4" : ""}`}>
            <div className="profile-card-header">
              <h2 className="profile-card-title">{t("profile.cloudflareTunnel", "Cloudflare Tunnel")}</h2>
            </div>
            ...
            <div className="profile-card-content">
              ...
              <div className="profile-info-section">
                <div className="profile-field">
                  <div className="profile-label">{t("profile.username", "Username")}</div>
                  <div className="profile-value">{cloudflareProfile.userName}</div>
                </div>

                <div className="profile-field">
                  <div className="profile-label">{t("profile.publicApiUrl", "Public API URL")}</div>
                  <div className="profile-value break-all text-sm">{cloudflareProfile.publicUrl}</div>
                </div>
